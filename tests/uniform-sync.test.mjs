...
import { UniformSync } from "../src/graphics/UniformSync.ts";

test("uniform sync uploads first use, in-place changes and new targets, but skips unchanged bytes", () => {
  const sync = new UniformSync(16), writes = [];
  const device = { queue: { writeBuffer: (buffer, offset, data) => writes.push([buffer, data.slice(0)]) } };
  const a = {}, b = {};
...
test("failed upload does not mark bytes as synchronized", () => {
  const sync = new UniformSync(4), target = {};
  let attempts = 0;
  const device = { queue: { writeBuffer() { if (++attempts === 1) throw Error("failed"); } } };
